...
class Geetestlogin(JiyanTrack):
    def __init__(self):
        ...
        with open("static/js/fullpage.js", "r", encoding="utf-8") as f:
            self.ctx = execjs.compile(f.read())
        with open("static/js/slide.js", "r", encoding="utf-8") as j:
            self.slide_str = j.read()
    ...
    def download_image(self, slice_url: str, bg_url: str):
        ...
        """
        try:
            res_slice = requests.get(url=self.host + slice_url)
            with open("static/images/slice.png", "wb") as f:
                f.write(res_slice.content)
            res_bg = requests.get(url=self.host + bg_url)
            # 还原背景图
    ...
    def get_validate(self, gt: str, challenge: str, s_s: str, url_gct: str, distance, passtimes, trace):
        ...
        try:
            ...
            res = self.session.get(url=self.host + url_gct)  # 下载gct.js文件
            api_payload = {
                "gt": gt,
                "challenge": challenge,
                "e": e_e,
