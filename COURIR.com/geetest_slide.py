...
class JiyanTrack(object):
    def __init__(self, bg_img_path, slice_img_path):
        self.bg_img_path = bg_img_path  # 背景图
        self.slice_img_path = slice_img_path  # 覆盖图

    def get_distance(self, bg_src, qk_src):
        """
        滑块验证
        :param bg_src: 背景图片
        :param qk_src: 缺口图片
        :@return:
        """
        success, distance = False, 0

        try:
            bg_img_obj = Image.open(bg_src)
            bg_img_obj = bg_img_obj.resize((260, 160))

            qk_img_obj = Image.open(qk_src)
            # qk_img_obj = qk_img_obj.resize((50, 40))

            distance, _ = self.get_position(bg_img_obj, qk_img_obj)
            success = True
    ...
    def get_position(self, target_img_obj, template_img_obj):
        """
        找出图像中最佳匹配位置
        :param target: 目标即背景图
        ...
        target_img_obj.save(target_output, format='PNG')
        target_hex_data = target_output.getvalue()
        target_buf = np.asarray(bytearray(target_hex_data), dtype="uint8")

        template_output = io.BytesIO()
        template_img_obj.save(template_output, format='PNG')
        template_hex_data = template_output.getvalue()
        template_buf = np.asarray(bytearray(template_hex_data), dtype="uint8")

        target_rgb = cv2.imdecode(target_buf, cv2.IMREAD_COLOR)
        target_gray = cv2.cvtColor(target_rgb, cv2.COLOR_BGR2GRAY)
        template_rgb = cv2.imdecode(template_buf, cv2.COLOR_BGR2BGRA)
        res = cv2.matchTemplate(target_gray, template_rgb, cv2.TM_CCOEFF_NORMED)
        value = cv2.minMaxLoc(res)
        return value[2:][0][0], value[2:][1][0]  # 采用value[2:][1][0]

    def get_trace_fast(self, distance):
    ...
    def get_trace_normal(self, distance):
    ...
    def choice_track_list(self, dist):
        source_track = [
    ...
    def choice_track(self, dist):
        track, tag = self.choice_track_list(dist)
        # 来自训练路径 tag=1
        # 规范化轨迹数据  [[x,y,t],...]
        ...
        if tag != 1:
            # 采用垃圾算法获取轨迹 建议重写
            new_track_list = self.get_trace_fast(dist)
        else:
            # tag==1 轨迹数据来自文件 直接赋值
    ...
    def get_distance_trace(self):
        distance = self.get_distance(self.bg_img_path, self.slice_img_path)
        trace = self.choice_track(distance[1])
        return distance[1], trace, trace[-1][-1]


class Geetestlogin(JiyanTrack):
    def __init__(self):
        super().__init__('static/images/bg.png', './static/images/slice.png')  # 背景图和滑块图
        self.session = requests.session()
        self.host = "https://api-na.geetest.com/"
        self.url_gettype = "https://apiv6.geetest.com/gettype.php"  # 非必要请求
        self.get = "https://api-na.geetest.com/get.php"
        self.url_ajax = "https://api-na.geetest.com/ajax.php"
    ...
    def get_bg_image(**kwargs):
        ...
        _ = 0
        # 图片拼接顺序
        location_list = [39, 38, 48, 49, 41, 40, 46, 47, 35, 34, 50, 51, 33, 32, 28, 29, 27, 26, 36, 37, 31, 30, 44, 45,
                         43,
                         42, 12, 13, 23, 22, 14, 15, 21, 20, 8, 9, 25, 24, 6, 7, 3, 2, 0, 1, 11, 10, 4, 5, 19, 18, 16,
                         17]
        im_list_upper = []
        im_list_down = []
        try:
            bg_img = Image.open(io.BytesIO(kwargs["img_byte"]))
            while _ < 52:
                x = location_list[_] % 26 * 12 + 1
                if location_list[_] > 25:
                    y = 80
                    im_list_upper.append(bg_img.crop((x, y, x + 10, 160)))
                else:
                    y = 0
                    im_list_down.append(bg_img.crop((x, y, x + 10, 80)))
                _ += 1
            new_img = Image.new("RGB", (260, 160))
            x_offset_upper = 0
            for img_upper in im_list_upper:
                new_img.paste(img_upper, (x_offset_upper, 0))
                x_offset_upper += img_upper.size[0]
            x_offset_down = 0
            for img_down in im_list_down:
                new_img.paste(img_down, (x_offset_down, 80))
                x_offset_down += img_down.size[0]
            new_img.save("./static/images/bg.png")
            logger.debug("图片还原成功")
    ...
    def download_image(self, slice_url: str, bg_url: str):
        """
        对滑块所需的图片进行下载,还原,保存
        @param slice_url: 滑块url
        @param bg_url: 背景url
        @return:
        """
        try:
            res_slice = requests.get(url="https://static.geetest.com/" + slice_url)
            with open("static/images/slice.png", "wb") as f:
                f.write(res_slice.content)
            res_bg = requests.get(url="https://static.geetest.com/" + bg_url)
            # 还原背景图
            self.get_bg_image(**{"img_byte": res_bg.content})
        except Exception as e:
    ...
    def get_s_c(self, e_e: str, gt: str, challenge: str) -> dict:
        ...
        try:
            ...
            if res_api["msg"] == "success":
                ...
                response = self.session.get(url=self.get, params=params)
    ...
    def get_material(self, gt: str, challenge: str) -> tuple:
        ...
        try:
            ...
            id = data_dict["id"]
            bg = data_dict["bg"]  # 背景
            slice = data_dict["slice"]  # 小块
            gct_path = data_dict["gct_path"]  # gct.js的地址
            return gt, challenge, s, id, bg, slice, gct_path
        except Exception as e:
            logger.error(f"参数解析失败 -> {response.text}")
            raise e
    ...
    def get_validate(self, gt: str, challenge: str, s_s: str, url_gct: str, distance, passtimes, trace):
        ...
        try:
            ...
            res = self.session.get(url="https://static.geetest.com/" + url_gct)  # 下载gct.js文件
            api_payload = {
                "gt": gt,
                "challenge": challenge,
                "e": e_e,
            ...
            if res_api["msg"] == "success":
                ...
                if response_dict["message"] == "success":
                    ...
                    return challenge, response_dict["validate"]
    ...
    def main(self, gt, challenge):
        try:
            ...
            gt_new, challenge_new, s_s, id, bg, slice, gct_path = self.get_material(gt, challenge)
            self.download_image(slice, bg)
            distance, trace, passtimes = self.get_distance_trace()
            challenge, validate = self.get_validate(gt_new, challenge_new, s_s, gct_path, distance, passtimes, trace)
